    ...
    for category in categories:
        ...
        for product in products:
            product_data = {
                ...
                'static_price': product.price,  # Local price for manually added products
                'is_amazon_product': product.is_amazon_product,
            }

            if product.is_amazon_product:
                # Fetch dynamic data from Amazon API
                amazon_data = fetch_amazon_data(product.amazon_id)
                if amazon_data:
                    product_data.update({
                        'price': amazon_data.get('price'),
                        'rating': amazon_data.get('rating'),
                        'reviews': amazon_data.get('reviews'),
                        'weight': amazon_data.get('weight'),
                    })
            else:
...
def search_suggestions(request):
    ...
    if query:
        ...
        products = Product.objects.filter(title__icontains=query, is_active=True)[:5]  # Limit to 5 results
        results = []
        for product in products:
            product_data = {
                'slug': product.slug,
